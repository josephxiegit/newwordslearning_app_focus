...
const isDictation = (item) => {
  // 1. 如果后端明确下发了标识
  if (item.默写 === true || item.排除 === '默写' || item.type === '默写') return true;
  
  // 2. 如果是手写、试题、拼写，则肯定不是中文默写题
  if (item.排除 === '手写' || item.排除 === '试题' || item.is_spell) return false;
  
  // 3. 智能推断：如果用户的答案不是“无”，且不在原有的中文选项列表中，说明这是上一页被抽中的主观填空题
  if (Array.isArray(item.用户选择) && item.用户选择.length > 0) {
    return item.用户选择.some(ans => {
      if (ans === '无') return false;
      if (Array.isArray(item.中文)) {
        return !item.中文.includes(ans);
      }
      return true;
    });
  }
    ...
    >
      ...
      <template #default>
        <div class="custom-content">
          ...
          <Divider></Divider>
          <div v-if="isBroken" style="color: red;"> 💔 收益减半，先完成回顾</div>
          <div
    ...
    >
      <van-cell-group>
        ...
        >
          ...
          <!-- 1. 默写/翻译题（不显示备选框，只展示填写的文本和答案） -->
          <div v-if="isDictation(item)" style="margin: 0; font-size: 15px; color: black;">
            <van-cell-group>
              <van-cell>
                <template #title>
              ...
              <van-cell :style="{ backgroundColor: item.flag === 'true' ? 'lightgreen' : (item.flag === 'half' ? 'orange' : 'bisque') }">
                <template #title>
